...
class Diklat extends CI_Controller
{
    ...
    public function simpan()
    {
        ...
        if ($this->ion_auth->in_group($group) && !empty($id_pegawai)) {
            ...
            $up['Master_Diklat_ID_Diklat']      = $this->input->post('id_master_diklat');
        ...
        if ($this->ion_auth->is_admin() && !empty($id_pegawai) && !empty($id_diklat)) {
            ...
            if ($data->num_rows() > 0) {
                foreach ($data->result() as $db) {
                    ...
                    $d['Master_Diklat_ID_Diklat']      = $db->Master_Diklat_ID_Diklat;
                    ...
                    $d['list_master_diklat'] = $this->diklat_model->get_diklat_by_jenis_display($d['Master_Diklat_ID_Diklat']);
                }
            } else {
                $d['Nama_Pegawai']                 = '';
                $d['ID_Diklat']                    = '';
    ...
    public function getDiklatByJenis($id_jenis)
    {
        $list_master_diklat = $this->diklat_model->get_diklat_by_jenis($id_jenis);
        foreach ($list_master_diklat->result() as $db) {
            echo "<option value=$db->ID_Diklat>$db->Nama_Diklat</option>";
        }
    }
