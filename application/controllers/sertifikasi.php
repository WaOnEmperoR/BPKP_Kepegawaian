...
class Sertifikasi extends CI_Controller
{
    ...
    public function simpan()
    {
        ...
        if ($this->ion_auth->in_group($group) && !empty($id_pegawai)) {
            ...
            $up['ID_Master_Sertifikasi']                  = $this->input->post('id_master_sertifikasi');
        ...
        if ($this->ion_auth->in_group($group) && !empty($id_pegawai) && !empty($id_sertifikasi)) {
            ...
            if ($data->num_rows() > 0) {
                foreach ($data->result() as $db) {
                    ...
                    $d['ID_Master_Sertifikasi']                  = $db->ID_Master_Sertifikasi;
                    ...
                    $d['list_master_sertifikasi'] = $this->sertifikasi_model->get_sertifikasi_by_jenis_display($d['Jenis_Sertifikasi_ID_Jenis_Sertifikasi']);
                }
            } else {
                $d['Nama_Pegawai']                           = '';
                $d['ID_Sertifikasi']                         = '';
    ...
    public function getSertifikasiByJenis($id_jenis)
    {
        $list_master_sertifikasi = $this->sertifikasi_model->get_sertifikasi_by_jenis($id_jenis);
        foreach ($list_master_sertifikasi->result() as $db) {
            echo "<option value=$db->ID_Sertifikasi>$db->Nama_Sertifikasi</option>";
        }
    }
