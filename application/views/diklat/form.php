            ...
            <div class="panel-body collapse in">
               <form id="form" enctype="multipart/form-data" action="<?php echo base_url(); ?>diklat/simpan/<?php echo $Pegawai_ID_Pegawai;?>" method="post" class="form-horizontal row-border">
                  ...
                  <div class="form-group">
                     ...
                     <div class="col-sm-6">
                        <select name="id_master_diklat" id="id_master_diklat" class="form-control" style="width: 200px;">
                           <option>--Pilih diklat--</option>
                           <?php 
                              foreach ($list_master_diklat as $lmd)
                              {
                              	$id_lmd = $lmd['ID_Diklat'];
                              	$nama_lmd = $lmd['Nama_Diklat'];
                              	echo "<option value = $id_lmd ";
                              	if ($id_lmd == $Master_Diklat_ID_Diklat)
                              	{
                              		echo "selected = 'selected' ";
                              	}
                              	echo("> $nama_lmd </option>");
                              }
                              ?>
                        </select>
                     </div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#jenis_diklat").change(function() {
			...
			$.ajax({
				type: "post",
				url: dasar + "diklat/getDiklatByJenis/" + jenis,
				success: function(data) {
					$("#id_master_diklat").html(data);
				}
			});
		});
	});
